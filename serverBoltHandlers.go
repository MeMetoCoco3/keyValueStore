...
func (b *BoltServer[K, V]) HandleDeleteEcho(c echo.Context) error {
	id, err := strconv.Atoi(c.Param("id"))
	if err != nil {
		log.Println("Error converting id:", err)
		return c.JSON(http.StatusBadRequest, map[string]string{"msg": "Invalid id"})
	...
	var user *User
	if boltStorer, ok := b.Storage.(BoltStorer[int, *User]); ok {
		user, err = boltStorer.DeleteB(id)
		if err != nil {
			log.Println("Error fetching user:", err)
			return c.JSON(http.StatusInternalServerError, map[string]string{"msg": "User not found"})
		}
	...
	return c.JSON(http.StatusOK, map[string]interface{}{"This user was deleted": json.RawMessage(buff)})
}
